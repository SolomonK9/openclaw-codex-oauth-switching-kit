    ...
    try:
        ...
        for ln in lines[-3000:]:
            ...
            if reason == "rate_limit":
                ...
                info["quarantined"].append({"profileId": pid, "reason": "rate_limit", "minutes": mins})
            elif reason == "timeout":
                info["timedOut"] += 1
                ...
                acc["quarantine"] = {
                    "active": True,
                    "until": until.isoformat(),
                    "reason": f"runtime_timeout:{timeout_quarantine_min}m",
                }
                append_history(state, {
                    "at": ts(),
                    "type": "runtime_failover_quarantine",
                    "profileId": pid,
                    "reason": "timeout",
                    "minutes": timeout_quarantine_min,
                })
                info["quarantined"].append({"profileId": pid, "reason": "timeout", "minutes": timeout_quarantine_min})
