...
if [[ -f "$KIT_DIR/scripts/oauth_routing_cli.py" ]]; then
  cp "$KIT_DIR/scripts/oauth_routing_cli.py" "$SCRIPTS_DIR/"
fi
if [[ -f "$KIT_DIR/scripts/openclaw_resolver.py" ]]; then
  cp "$KIT_DIR/scripts/openclaw_resolver.py" "$SCRIPTS_DIR/"
fi
cp "$KIT_DIR/scripts/setup_oauth_timers.sh" "$SCRIPTS_DIR/"
cp "$KIT_DIR/scripts/setup_oauth_crons.sh" "$SCRIPTS_DIR/"
...
PY_FILES=(
  "$SCRIPTS_DIR/oauth_pool_router.py"
  "$SCRIPTS_DIR/oauth_command_router.py"
  "$SCRIPTS_DIR/oauth_lease_sync.py"
  "$SCRIPTS_DIR/oauth_profile_capture.py"
  "$SCRIPTS_DIR/onboard_oauth_account.py"
  "$SCRIPTS_DIR/oauth_telegram_reauth.py"
  "$SCRIPTS_DIR/oauth_telegram_bridge.py"
)
if [[ -f "$SCRIPTS_DIR/oauth_routing_cli.py" ]]; then
  PY_FILES+=("$SCRIPTS_DIR/oauth_routing_cli.py")
fi
if [[ -f "$SCRIPTS_DIR/openclaw_resolver.py" ]]; then
  PY_FILES+=("$SCRIPTS_DIR/openclaw_resolver.py")
fi
...
echo "4) Preferred automation: $SCRIPTS_DIR/setup_oauth_timers.sh $WORKSPACE"
echo "   Fallback only: $SCRIPTS_DIR/setup_oauth_crons.sh $WORKSPACE"
