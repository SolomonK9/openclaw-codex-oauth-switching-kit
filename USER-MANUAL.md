...
- live 429/rate-limit cooldowns that do not poison longer-window capacity telemetry
...
Preferred path: direct systemd user timers. This runs the scripts directly and avoids high-frequency OpenClaw cron/LLM wakeups.
...
Fallback only if systemd user timers are unavailable: run `./scripts/setup_oauth_crons.sh ~/.openclaw/workspace`. Do not install both timer and cron automation.

Default automation:
- OAuth Pool Router Tick
- OAuth Lease Sync

Optional hardening jobs are intentionally not auto-installed in this public kit. Add them only after the core system is working cleanly in your environment.
...
- temporary 429/rate-limit failures appear as cooldown/throttle pressure, not permanent quota exhaustion
...
If you are giving the kit to another OpenClaw agent, paste `CLAW-INSTALL-PROMPT.md` into that agent.
